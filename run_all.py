...
import cv2, numpy as np, math, time, json, os, threading, subprocess, sys, logging
...
logger = logging.getLogger("LineLens")
...
try:
    ...
    HAS_LEDS = False
    logger.warning("Could not load LED buzzer script (alerts.led_buzzer)")
...
JITTER_THRESH = 3.0   # minimum pixels moved to count as actual walking
SMOOTH_FACTOR = 0.7   # EMA smoothing factor (0.0 - 1.0) for centroid tracking
...
# load video source
video_path = './test_videos/worker-zone-detection.mp4'
...
try:
    ...
    has_zones = True
    logger.info(f"Loaded {len(polys)} zones from config")
except Exception as e:
    has_zones = False
    polys = {}
    logger.warning("No zones loaded (run calibration/zone_drawing.py first)")
...
def save_clip_async(event_name, frames_list):
    ...
    def write_video():
        try:
            ...
            vw.release()
            logger.info(f"[Clip Saved] {fname}")
        except Exception as e:
...
def save_session_data():
    ...
    try:
        ...
        if heatgrid.max() > 0:
            ...
            logger.info("Saved heatmap to data/heatmap.png")
        ...
        logger.info("Saved spaghetti diagram to data/spaghetti_diagram.png")
        logger.info("Successfully saved all data to data/ folder!")
    ...
    logger.info("Auto-generating shift report (calling Groq for fixes)...")
    try:
        ...
        if _result.stdout: logger.info(f"Shift Report Output:\n{_result.stdout}")
        if _result.returncode != 0:
            logger.error(f"Shift report error: {_result.stderr[:500]}")
    except Exception as e:
        logger.error(f"Failed to run shift report script: {e}")

logger.info("====== LineLens AI Running ======")
logger.info("  MULTI-PERSON YOLO + FULL LOGIC")
logger.info("  q = quit and save reports")
logger.info("  h = toggle heatmap")
logger.info("=================================")
...
while True:
    try:
        ...
        if ts - last_upload_time > 24 * 3600:
            logger.info("24 hours elapsed, auto-saving and uploading...")
            save_session_data()
            last_upload_time = ts
        ...
        # --- simple tracker with EMA smoothing ---
        matched_ids = []
        for (cx, cy) in current_centroids:
            ...
            if best_id is not None:
                p = active_persons[best_id]
                # EMA Smoothing
                old_x, old_y = p.pos
                new_x = old_x * SMOOTH_FACTOR + cx * (1.0 - SMOOTH_FACTOR)
                new_y = old_y * SMOOTH_FACTOR + cy * (1.0 - SMOOTH_FACTOR)
                p.pos = (new_x, new_y)
        ...
        for i, pid in enumerate(matched_ids):
            ...
            # 1. idle and walking (Jitter thresholding)
            if len(p.trail) == 0:
                p.trail.append((int(p.pos[0]), int(p.pos[1])))
            else:
                last_trail = p.trail[-1]
                dist_moved = math.hypot(p.pos[0]-last_trail[0], p.pos[1]-last_trail[1])
                
                # Walk distance logic - only count if > JITTER_THRESH
                if dist_moved > JITTER_THRESH:
                    curr = (int(p.pos[0]), int(p.pos[1]))
                    all_walk_paths.append((last_trail, curr))
                    p.total_walk += dist_moved
                    p.trail.append(curr)
                
                # Idle logic: use an anchor position to prevent slow-walking bypass
                idle_dist = math.hypot(p.pos[0]-p.idle_pos[0], p.pos[1]-p.idle_pos[1])
                
                if idle_dist < IDLE_THRESH * 5:
                    if p.idle_start is None:
                        p.idle_start = ts
                        p.idle_pos = p.pos
                    dur = ts - p.idle_start
                    if dur > IDLE_TIME:
                    ...
                    if current_z:
                        ...
                        if len(EXPECTED_SOP) > 0 and current_z == EXPECTED_SOP[0] and len(p.zone_history) > 1:
                            ...
                            if dur > 10.0:
                                ...
                                logger.info(f"Cycle finished! P{pid} | Dur: {dur:.1f}s | Drift: {drift:.1f}%")
            ...
            if bad_posture and len(violations) % 10 == 1:
                fire_alert("ergo")
...
save_session_data()
logger.info("LineLens AI Shutdown Complete.")
